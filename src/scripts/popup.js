...
function renderOptions(settings) {
  ...
  // old code ... is rows.join('') an string?!
  // $("#menuInsertPoint").html(rows.join(''));
  document.querySelector('#menuInsertPoint').textContent = rows.join('');
...
function installListeners(ids) {
  ...
  [...document.querySelector('#patterns, #disabled, #' + ids.join(',#'))].forEach(item => {
